...
def mnr_calculate_fuzzy_values(r, schema_data):
    for n, j in schema_data.iterrows():
        # House Number
        query = j.searched_query
        hnr_mt = (fuzz.token_set_ratio(j.hsn, query))
        query = str(query).replace(j.hsn, '',1).lstrip()
        # Street Name
        sn_mt = (fuzz.token_set_ratio(j.street_name, query))
        query = str(query).replace(j.street_name, '',1).lstrip()
        # Place Name
        pln_mt = (fuzz.token_set_ratio(j.place_name, query))
        query = str(query).replace(j.place_name, '',1).lstrip()
        # Postal Code
        pcode_mt = (fuzz.token_set_ratio(j.postal_code, query))
        query = str(query).replace(j.postal_code, '',1).lstrip()
