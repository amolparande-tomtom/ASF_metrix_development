...
countries_gdf = gpd.read_file(input_path, layer='10', encoding="utf-8")
# postgres Connection
...
filename = 'FRA_batch1_Delta_Source_10.csv'
...
for i, r in countries_gdf.iterrows():
    ...
    buffer = 0.00001
    # if r.UNIQUE_ID == '77316_o5ozxm_00049':
    ...
    for n, c in mnr_schema_data.iterrows():
        ...
        mnr_schema_data.loc[n, 'geographic_code_name_match%'] = (mnr_schema_data['geographic_code_name_match'][n] / 100)
    ...
    if not mnr_schema_data.empty:
        ...
        if add_header:
            mnr_schema_data.to_csv(outputpath + filename, mode='w', index=False)
            add_header = False
        else:
            mnr_schema_data.to_csv(outputpath + filename, mode='a', header=False, index=False)
    else:
...
if len(empty_data) > 0:
    ...
    missing_data.to_file(outputpath+'FRA_batch1_Delta_Source_FRA_10.csv.gpkg', layer='cities', driver="GPKG" )
