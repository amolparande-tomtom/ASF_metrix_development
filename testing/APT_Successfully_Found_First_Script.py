...
def csv_buffer_db_apt_vad_fuzzy_matching(csv_gdf, vad_schema_name, db_url, outputpath, filename):
    for i, r in csv_gdf.iterrows():
        ...
        if not schema_data.empty:
            ...
            #     len(row)
            #     pass
                # if row.new_hsn != 0 and row.new_street_name != 'NODATA' and row.new_postal_code != 0 and row.new_place_name != 'NODATA':
            if i == 0:
...
vad_filename = '1111_VAD_Distance_Fuzzy_ASF_output_BEL_NL.csv'
